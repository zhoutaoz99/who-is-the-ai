...
export class AiService {
  ...
  private buildSpeechPrompt(context: GameContext): string {
    ...
    parts.push(`当前轮次：第${context.roundNo}轮`);
    parts.push(
      `剩余时间：${Math.ceil(context.remainingTimeMs / 1000)}秒`,
    );
    parts.push(
      `存活玩家：${context.alivePlayers.map((p) => `${p.name}(ID:${p.id},座位${p.seatNo})`).join("、")}`,
    );
  ...
  private buildVotePrompt(
    ...
    parts.push(`当前轮次：第${context.roundNo}轮（投票阶段）`);

    const targets = context.alivePlayers.filter((p) => p.id !== aiPlayerId);
    parts.push(
      `可投票目标：${targets.map((p) => `${p.name}(ID:${p.id},座位${p.seatNo})`).join("、")}`,
    );
