...
- `AI-Prompt-Eval-Auto-Optimize.md`：看“自动优化器怎么跑”。以后改自动优化器实现逻辑，优先只改这一篇。
- `AI-Prompt-Eval-Details.md`：看“怎么算”。重点是版本库、单局打分和 scorecard 聚合。

建议阅读顺序：

1. 先看 Flow，建立整体路径。
2. 如果要改自动优化器，直接看 Auto-Optimize。
3. 再看 Details，补齐版本库、打分和聚合细节。
...
| [AI-Prompt-Eval-Auto-Optimize.md](./AI-Prompt-Eval-Auto-Optimize.md) | 自动优化器的独立维护点，轮后状态、请求重建、重试与日志。 |
| [AI-Prompt-Eval-Details.md](./AI-Prompt-Eval-Details.md) | AI 提示词版本库、评估尺子版本库、单局打分、轮聚合。 |
...
- 改自动优化器实现逻辑、状态重建、重试和日志时，先看 Auto-Optimize。
- 改 prompt 版本管理、打分 JSON、聚合公式时，先看 Details。
