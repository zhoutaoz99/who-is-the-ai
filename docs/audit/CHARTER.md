...
| 关联文档 | [文档规范](../Doc-Style-Guide.md)、[总体设计](../design/总体设计文档%20架构与设计依据.md)、[对照测试模块](../design/离线优化沙盒/模块详细设计/4.对照测试模块%20·%20流水线自检方案设计.md) |
...
- 提示：产品对局逐轮 LLM I/O 落在 `ai_call_logs`（已落盘，🟢 层）；**沙盒**对局的逐轮 LLM I/O 仍为内存缓冲（🟡 层，见 §4.4）。
...
并非所有中间数据都已留存。agent **必须**先判断目标数据属于哪一层，再决定能否据此下结论：

| 层 | 数据 | 现状 | agent 处理 |
| --- | --- | --- | --- |
| 🟢 已落盘 | MatchRecord、ScoreRecord、GenerationEval、编排器状态、版本 | 任何时刻可直读（§4.1 底座） | 直接读取取证 |
| 🟡 临时 | 逐轮 LLM 原始 I/O（`observability.ts` 内存环形缓冲 `MAX_RECORDS=1000`，重启即丢）、裁判 LLM 原文、live run 态 | 仅当场/近窗可读，旧数据丢失 | 当场跑当场读；缺失时标注“需开启 trace 留存”，不臆断 |
| 🔴 未留存 | 聚合中间产物（buckets / validation / weak-dims / failures）、优化器内部 | 算完即弃，未持久化 | 当前**无法**直接验证；记为“需埋点（见 §13 trace bundle）”，不得凭空判定为缺陷 |

| ID | 规则 |
| --- | --- |
| `DATA-NO-FABRICATE` | 当某项验证依赖 🟡/🔴 层、而该数据本次未被捕获时，agent **必须**报成“待留存/待埋点”的覆盖缺口，**不得**据缺失数据反推实现有 bug |
...
- 实现“审计跑 trace bundle”，补齐 §4.4 🟡/🔴 两层中间数据的可达性。待埋点清单：
  - 🟡 逐轮 LLM 原始 I/O 按 `match_id` 落盘（`observability.ts` 现为进程内环形缓冲）。
  - 🔴 聚合中间产物（buckets / validation / weak-dims / failures）、裁判 LLM 原文、优化器内部，在 trace 模式下 emit。
  - 收口为按 `run_id` 维度的 trace bundle + manifest，供 agent 按需拉取；trace 默认关，仅审计跑开启。
